...
class CorefDocument:
    ...
    @classmethod
    def from_parent(cls, parent, i, j):
        """Create a subdocument from a parent CorefDocument."""
        subdoc = cls()
        subdoc.movie = f"{parent.movie}_{len(parent.token[i:j])}"
        subdoc.token = parent.token[i:j]
        subdoc.parse = parent.parse[i:j]
        subdoc.parse_ids = parent.parse_ids[i:j]
        subdoc.pos = parent.pos[i:j]
        subdoc.pos_ids = parent.pos_ids[i:j]
        subdoc.ner = parent.ner[i:j]
        subdoc.ner_ids = parent.ner_ids[i:j]
        subdoc.speaker = parent.speaker[i:j]
        subdoc.is_pronoun = parent.is_pronoun[i:j]
        subdoc.is_punctuation = parent.is_punctuation[i:j]
        subdoc.word_cluster_ids = parent.word_cluster_ids[i:j]
        subdoc.word_head_ids = parent.word_head_ids[i:j]
        subdoc.sentence_offsets = [
            offset for offset in parent.sentence_offsets
            if offset[0] >= i and offset[1] < j
        ]
        subdoc.offset = (i, j)
        return subdoc
