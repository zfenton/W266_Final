...
class MovieCoreference:
    ...
    def _split_screenplay(self, document: data.CorefDocument, split_len: int, overlap_len: int, 
        ...
        if not document.token:
            print(f"Document {document.movie} has no tokens. Skipping...")
            return
        ...
        for i in range(len(document.token)):
            if document.parse[i] in "SNC":
                segment_boundaries[i] = 1
        segment_boundaries[0] = 1  # Ensure first token is always a boundary
        segment_boundaries[-1] = 1  # Ensure last token is always a boundary

        if not np.any(segment_boundaries):
            print(f"No segment boundaries for document {document.movie}. Using defaults.")
            segment_boundaries[0] = 1
            segment_boundaries[-1] = 1

        # Adjust split_len if the document is too short
        if len(document.token) < split_len:
            split_len = len(document.token)
        ...
        while i < len(document.token):
            ...
            if j < len(document.token):
                ...
                nexti = j
            if i >= nexti:
                print(f"Skipping empty segment for document {document.movie} at i={i}, nexti={nexti}")
                break
        ...
        # Validate overlaps
        for idx in range(len(doc_offsets) - 2):
            i0, j0 = doc_offsets[idx]
            i1, j1 = doc_offsets[idx + 1]
            i2, j2 = doc_offsets[idx + 2]
            if not (i0 < i1 <= j0 < i2 <= j1 < j2):
                print(f"Invalid overlap for document {document.movie}: {doc_offsets}")
                raise AssertionError(f"Too many overlaps: doc={document.movie}, doc_offsets={doc_offsets}")

        # Split screenplay into subdocuments
        # for k, (i, j) in enumerate(doc_offsets):
        #     subdoc = data.CorefDocument()
        #     subdoc.movie = f"{document.movie}_{k + 1}"
        #     subdoc.token = document.token[i:j]
        #     subdoc.parse = document.parse[i:j]
        #     subdoc.parse_ids = document.parse_ids[i:j]  # Propagate parse_ids
        #     subdoc.pos_ids = document.pos_ids[i:j]  # Propagate pos_ids
        #     subdoc.ner = document.ner[i:j]
        #     subdoc.ner_ids = document.ner_ids[i:j]  # Propagate ner_ids
        #     subdoc.is_pronoun = document.is_pronoun[i:j]  # Propagate is_pronoun
        #     subdoc.is_punctuation = document.is_punctuation[i:j]
        #     subdoc.word_cluster_ids = document.word_cluster_ids[i:j]
        #     subdoc.word_head_ids = document.word_head_ids[i:j]
        #     subdoc.sentence_offsets = sentence_offsets[
        #         (sentence_offsets[:, 0] >= i) & (sentence_offsets[:, 1] < j)
        #     ].tolist()
        #     subdoc.offset = (i, j)
                # if exclude_subdocuments_with_no_clusters and not subdoc.clusters:
                #         continue

                #     yield subdoc

        for k, (i, j) in enumerate(doc_offsets):
            subdoc = document.from_parent(document, i, j)
    ...
    def _create_cr_dataloader(self, word_embeddings: torch.FloatTensor, document: data.CorefDocument,
        ...
        if not hasattr(document, "parse"):
            raise ValueError(f"Document {document.movie} is missing parse before sequence generation.")
        
        if not hasattr(document, "parse_ids"):
            raise ValueError(f"Document {document.movie} is missing parse_ids before sequence generation.")
        ...
        for document, obj in tqdm.tqdm(zip(corpus, movie_data), desc="coref prediction", total=len(corpus)):
            ...
            # fusion-based
                ...
                subdocument_corpus = self._prepare_corpus(test_corpus, self.document_len, self.overlap_len,
                                                        exclude_subdocuments_with_no_clusters=False)
                for subdocument in subdocument_corpus:
                    # print(f"Subdocument: {subdocument.movie}")
                    for attr in ["subword_dataloader", "word_to_subword_offset", "parse_ids"]:
                        if not hasattr(subdocument, attr) or getattr(subdocument, attr) is None:
                            print(f"Missing or uninitialized attribute: {attr} in {subdocument.movie}")
